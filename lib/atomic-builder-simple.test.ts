...
import { ValueType, RelationType } from "./types.ts";
...
Deno.test("Atomic Builder - Working Functionality", async (t) => {
  ...
  await t.step("should handle empty mutations commit", async () => {
    ...
    assertEquals(result.failedMutation.error.message, "commit operation failed: No mutations to commit");
  });
  ...
  await t.step("should handle too many mutations", async () => {
    ...
    assertEquals(result.failedMutation.index, -1);
    assertExists(result.failedMutation.error);
    assertEquals(result.failedMutation.error.message.includes("Too many mutations"), true);
  });
  ...
  await t.step("should test update validation with non-existent record", async () => {
    const builder = createAtomicBuilder(kv);
    
    // Try to update non-existent user
    builder.update(User.entity, { id: "atomic_nonexistent_user" }, { name: "Updated" });
  ...
  await t.step("should test delete validation with non-existent record", async () => {
    const builder = createAtomicBuilder(kv);
  ...
  await t.step("should test update with merge option", async () => {
    ...
    builder.update(User.entity, { id: "atomic_merge_user" }, { name: "Updated Name" }, { 
      merge: true 
    });
  ...
  await t.step("should test update validation with invalid merged data", async () => {
    const builder = createAtomicBuilder(kv);
  ...
  await t.step("should test timeout and retry behavior on failures", async () => {
    const builder = createAtomicBuilder(kv);
  ...
  await t.step("should test mutation validation errors", async () => {
    ...
    assertEquals(result.ok, false);
    assertExists(result.failedMutation);
    assertEquals(result.failedMutation.error.message.includes("Create mutation requires entity and data"), true);
  });
  ...
  await t.step("should test update mutation validation errors", async () => {
    ...
    assertEquals(result.ok, false);
    assertExists(result.failedMutation);
    assertEquals(result.failedMutation.error.message.includes("Update mutation requires entity, key, and data"), true);
  });
  ...
  await t.step("should test delete mutation validation errors", async () => {
    ...
    assertEquals(result.ok, false);
    assertExists(result.failedMutation);
    assertEquals(result.failedMutation.error.message.includes("Delete mutation requires entity and key"), true);
  });
  ...
  await t.step("should test entity with VALUE type secondary index", async () => {
    const builder = createAtomicBuilder(kv);
  ...
  await t.step("should test update operations with secondary indexes", async () => {
    const builder = createAtomicBuilder(kv);
  ...
  await t.step("should test delete operations with secondary indexes", async () => {
    const builder = createAtomicBuilder(kv);
  ...
  await t.step("should test cascade delete functionality", async () => {
    ...
    builder.delete(Article.entity, { id: "article_cascade" }, { cascadeDelete: true });
  ...
  await t.step("should test creating record with existing primary key during validation", async () => {
    const builder = createAtomicBuilder(kv);
  ...
  await t.step("should test duplicate key validation during commit", async () => {
    const builder = createAtomicBuilder(kv);
    ...
    if (!result.ok) {
      assertExists(result.failedMutation);
      assertEquals(result.failedMutation.error.message.includes("Record already exists"), true);
    }
  });
  ...
  await t.step("should test actual commit failure causing retries", async () => {
    const builder = createAtomicBuilder(kv);
  ...
  await t.step("should test successful update operation execution", async () => {
    const builder = createAtomicBuilder(kv);
  ...
  await t.step("should test successful delete operation execution", async () => {
    const builder = createAtomicBuilder(kv);
  ...
  await t.step("should test successful check/sum/min/max operations execution", async () => {
    const builder = createAtomicBuilder(kv);
