...
import { AtomicCounter, AtomicUtils, ModelAtomicUtils } from "./atomic-utils.ts";
...
Deno.test("AtomicCounter", async (t) => {
  ...
  await t.step("increment - increments counter by specified amount", async () => {
    const kv = await Deno.openKv(":memory:");
    const counter = new AtomicCounter(kv, ["test", "counter"]);
  ...
  await t.step("decrement - decrements counter by specified amount", async () => {
    const kv = await Deno.openKv(":memory:");
    const counter = new AtomicCounter(kv, ["test", "counter"]);
  ...
  await t.step("conditionalIncrement - increments when condition matches", async () => {
    const kv = await Deno.openKv(":memory:");
    const counter = new AtomicCounter(kv, ["test", "counter"]);
  ...
  await t.step("conditionalIncrement - does not increment when condition does not match", async () => {
    const kv = await Deno.openKv(":memory:");
    const counter = new AtomicCounter(kv, ["test", "counter"]);
  ...
  await t.step("conditionalIncrement - increments by specified amount", async () => {
    const kv = await Deno.openKv(":memory:");
    const counter = new AtomicCounter(kv, ["test", "counter"]);
...
Deno.test("ModelAtomicUtils", async (t) => {
  ...
  await t.step("createFieldCounter - creates counter with string record key", async () => {
    const kv = await Deno.openKv(":memory:");
  ...
  await t.step("createFieldCounter - creates counter with object record key", async () => {
    const kv = await Deno.openKv(":memory:");
    const utils = new ModelAtomicUtils(kv, userEntity);
  ...
  await t.step("incrementFields - increments multiple fields atomically", async () => {
    const kv = await Deno.openKv(":memory:");
    const utils = new ModelAtomicUtils(kv, userEntity);
  ...
  await t.step("getCounters - retrieves all counter values for a record", async () => {
    const kv = await Deno.openKv(":memory:");
    const utils = new ModelAtomicUtils(kv, userEntity);
  ...
  await t.step("getCounters - returns empty object for record with no counters", async () => {
    const kv = await Deno.openKv(":memory:");
    const utils = new ModelAtomicUtils(kv, userEntity);
