...
async function main() {
  ...
  try {
    ...
    const userSchema = await import("zod").then(z => z.z.object({
      id: z.z.string(),
      name: z.z.string(),
    ...
    await User.create({ id: "user1", name: "John Doe", email: "john@example.com", age: 25 });
    await User.create({ id: "user2", name: "Jane Smith", email: "jane@example.com", age: 30 });
    await User.create({ id: "user3", name: "Bob Johnson", email: "bob@example.com", age: 35 });
    ...
    const result = await kvm.migrate({
      ...
      onAfterMigration: (migration, result) => {
        if (result.success) {
          console.log(`✅ Completed migration ${migration.version} (${result.duration}ms)`);
        } else {
          console.log(`❌ Failed migration ${migration.version}: ${result.error?.message}`);
        }
      }
    });
    ...
    console.log(`Total applied migrations: ${stats.storage.totalAppliedMigrations}`);
    ...
    console.log(`Rolled back from version ${rollbackResult.previousVersion} to ${rollbackResult.currentVersion}`);
    console.log(`Rollback migrations: ${rollbackResult.executedMigrations.length}`);
